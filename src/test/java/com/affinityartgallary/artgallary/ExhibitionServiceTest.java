...
public class ExhibitionServiceTest {
    ...
    @Test
    void testThatExhibitionCanBeAddedToArtist(){
        artistService.addArtist(addArtistRequest);
        Artist foundArtist = artistService.getArtistByName("Ben");
        Exhibition exhibition = exhibitionService.addExhibition(foundArtist.getId(),addExhibitionRequest);
        assertEquals(exhibition.getArtistId(),foundArtist.getId());

    }
    @Test
    void testThatAnExhibitionCanBeFoundByArtistId(){
        artistService.addArtist(addArtistRequest);
        Artist foundArtist = artistService.getArtistByName("Ben");
        Exhibition exhibition = exhibitionService.addExhibition(foundArtist.getId(),addExhibitionRequest);
    ...
    @Test
    void testThatExhibitionCanBeUpdated(){
        artistService.addArtist(addArtistRequest);
        Artist foundArtist = artistService.getArtistByName("Ben");
        Exhibition exhibition = exhibitionService.addExhibition(foundArtist.getId(),addExhibitionRequest);
       Exhibition updatedExhibition =  exhibitionService.updateExhibition(exhibition.getId(),updateExhibitionRequest);
        assertEquals(updatedExhibition.getExhibitionName(),"The things left unsaid,2021");
    }
    @Test
    void testThatExhibitionCanBeDeleted(){
        artistService.addArtist(addArtistRequest);
        Artist foundArtist = artistService.getArtistByName("Ben");
        Exhibition exhibition = exhibitionService.addExhibition(foundArtist.getId(),addExhibitionRequest);
